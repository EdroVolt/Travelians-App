...
function updateUI(locationImage, weatherData) {
    ...
    divInfo.innerHTML = `location: ${weatherData.city_name} <br> temp: ${weatherData.app_temp}` +
...
async function getInformations() {
    ...
    const dayes = Math.round(Math.abs(new Date() - new Date(date.replace(/-/g, '/'))) / (1000 * 60 * 60 * 24));
    console.log(dayes)
